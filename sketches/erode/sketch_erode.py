...
class ErodeSketch(vsketch.SketchClass):
    ...
    def draw(self, vsk: vsketch.Vsketch) -> None:
        ...
        grid_spacing_total = (grid_count - 1) * self.grid_spacing

        cell_width = (available_width - grid_spacing_total) / grid_count
        cell_height = (available_height - grid_spacing_total) / grid_count
        ...
        for cell in range(grid_count * grid_count):
            row = cell // grid_count
            column = cell % grid_count
            cell_x = self.margin + column * (square_size + self.grid_spacing)
            cell_y = self.margin + row * (square_size + self.grid_spacing)
            ...
            for line_index in range(line_interval_count + 1):
                ...
                x_start = x_min if is_even else x_max
                x_end = x_max if is_even else x_min
